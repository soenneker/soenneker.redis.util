...
public interface IRedisUtil
{
    ...
    /// </summary>
    ValueTask<T?> Get<T>(string redisKey) where T : class;
    
    /// <summary>
    /// Includes deserialization of the redisValue. Will return null if error deserializing. Key is optional.
    /// </summary>
    ValueTask<T?> Get<T>(string cacheKey, string? key) where T : class;

    /// <summary>
    /// Key is optional.
    /// </summary>
    [Pure]
    ValueTask<string?> GetString(string cacheKey, string? key);

    /// <summary> Base method for all Get() methods. Does not build a redis key. </summary>
    [Pure]
    ValueTask<string?> GetString(string redisKey);

    [Pure]
    ValueTask<string?> GetHash(string redisKey, string field);

    /// <summary>
    /// Includes deserialization of the redisValue. Will return null if error deserializing.
    /// </summary>
    ValueTask<T?> GetHash<T>(string redisKey, string field) where T : class;


    /// <summary> Includes serialization of the object. Key is optional. </summary>
    ValueTask Set<T>(string cacheKey, string? key, T value, TimeSpan? expiration = null, bool useQueue = false) where T : class;

    /// <summary> Includes serialization of the object. </summary>
    ValueTask Set<T>(string redisKey, T value, TimeSpan? expiration = null, bool useQueue = false) where T : class;
    ...
    ValueTask Set(string redisKey, string redisValue, TimeSpan ? expiration = null, bool useQueue = false);

    ValueTask SetHash(string redisKey, string field, string redisValue, bool useQueue = false);

    /// <summary> Key is optional. </summary>
    ValueTask Remove(string cacheKey, string? key, bool useQueue = false);

    /// <summary> Base method for Remove()</summary>
    ValueTask Remove(string redisKey, bool useQueue = false);
}